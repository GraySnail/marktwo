...
class Doc extends React.Component {
  ...
  initializeFromDocList(docList, caretAt) {
    if (docList.filter(d => !d).length === 0) {
      ...
      document.querySelector('#m2-doc').innerHTML = visibleDocList
        .map(entry => this.getNodeForBlock(entry.text)[0].outerHTML)
        .join('\n')
  ...
  getDocList(docMetadata) {
    if (this.props.initialData) {
      ...
      return new Promise(resolve => {
        this.syncUtils
          .findOrFetchFiles(docMetadata.pageIds)
          .then(pages => {
            if (pages.length) {
  ...
  getAllLines() {
    ...
    const linesAfter = _.slice(
      this.state.allLines,
      Math.min(this.state.endIndex, this.state.allLines.length),
      this.state.allLines.length
    )
  ...
  sync(lines, forceSync) {
    ...
    for (i = 0; i < docMetadata.pageIds.length; i++) {
      const page = _.slice(lines, startIndex, startIndex + docMetadata.pageLengths[i]).map(id => ({
        id,
        text: this.state.doc[id],
      }))
    ...
    for (let j = docMetadata.pageIds.length - 1; j > i; j--) {
      const page = _.slice(lines, endIndex - docMetadata.pageLengths[j], endIndex).map(id => ({
        id,
        text: this.state.doc[id],
      }))
      const hash = md5(stringify(page))
    ...
    return this.syncUtils
      ...
      .then(results => {
        ...
        return new Promise((resolve, reject) => {
          this.syncUtils
            .syncByRevision(this.props.currentDoc, docMetadata)
            .then(validatedDocMetadata => {
              if (this._isMounted) {
                ...
                if (!_.isEqual(docMetadata.pageIds, validatedDocMetadata.pageIds)) {
                  ...
                  this.getDocList(validatedDocMetadata).then(docList =>
                    this.initializeFromDocList(docList, validatedDocMetadata.caretAt)
                  )
                }
              }
            })
  ...
  enterEditMode() {
    ...
    const originalAnchorText =
      sel.anchorNode && sel.anchorNode.data ? sel.anchorNode.data.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&') : 0
  ...
  handleScroll() {
    ...
    if (scrollPercent > 0.9 && endIndex < this.state.allLines.length) {
      ...
      const newHtml = _.slice(this.state.allLines, oldEndIndex, endIndex)
        .map(id => {
    ...
    if (scrollPercent < 0.1 && startIndex > 0) {
      ...
      const newHtml = _.slice(this.state.allLines, startIndex, oldStartIndex)
        .map(id => {
          const newBlock = this.getNodeForBlock(this.state.doc[id])
          newBlock.attr('id', id)
          return newBlock[0].outerHTML
    ...
    if (endIndex - startIndex > 500) {
      ...
      if (scrollPercent > 0.8) {
        document
          .querySelectorAll(`#m2-doc > *:nth-child(-n+${document.querySelectorAll('#m2-doc > *').length - 250})`)
          .forEach(el => {
            el.remove()
          })
  ...
  initializeEditor() {
    ...
    modal.querySelector('input').addEventListener('change', function (e) {
      ...
      if (e.target.files && e.target.files[0]) {
        ...
        reader.onload = function (e) {
          ...
          that.syncUtils.createImage(`m2img.${fileName}`, e.target.result).then(result => {
            console.log(result)
    ...
    $('#m2-doc').on('keydown keyup mouseup', e => {
      ...
      const s =
        sel.anchorNode && sel.anchorNode.data && sel.anchorNode.data.substring(sel.anchorOffset - 50, sel.anchorOffset)
      ...
      if (autocompleteRegex.test(s)) {
        ...
        if (matchedText.startsWith(':')) {
          ...
          results = _.uniq(
            _.flatten(
              this.state.allLines
                .map(id => id !== selectedBlock[0].id && this.state.doc[id].match(findRegex))
                .filter(r => r)
            )
          )
        }
        ...
        autocompleteSelectedIndex &&
          $(`#m2-autocomplete div:nth-child(${autocompleteSelectedIndex})`).addClass('m2-selected')
        ...
        function selectEntry() {
          ...
          const startOffset = endOfWord
            ? endOffset - (matchedText.length + endOfWord[0].length)
        ...
        if (selectedBlock && selectedBlock[0] && !(autocompleteSelectedIndex > 0)) {
          ...
          if (
            ...
            const initialContent =
              ...
              sel.anchorNode.nextSibling.data &&
              sel.anchorNode.nextSibling.data.replace(/\u200B/g, '').trim()
            const id = shortid.generate()
      ...
      <div>
        ...
        <dialog id="m2-img-dialog" className="content">
          ...
          <div className="actions is-pulled-right">
            <button id="m2-img-cancel" className="button is-text">
              cancel
            </button>
            <button id="m2-img-select" className="button" disabled>
              Ok
            </button>
          </div>
        </dialog>
        ...
        <dialog id="m2-date-dialog" className="content">
          ...
          <div className="actions is-pulled-right">
            <button id="m2-date-cancel" className="button is-text">
              cancel
            </button>
            <button id="m2-date-select" className="button">
              Ok
            </button>
          </div>
        </dialog>
        <div id="m2-autocomplete" style={{ display: 'none' }}></div>
        <div className="m2-offline" style={{ display: 'none' }}>
          <FontAwesomeIcon icon={faBolt} />
        </div>
        <div className="m2-is-signed-out" style={{ display: 'none' }}>
          You've been signed out. <a onClick={this.props.handleLogin}>Sign back in</a>
        </div>
        ...
        <div id="m2-doc" className="m2-doc content" contentEditable="true"></div>
      </div>
    )
  }
}
