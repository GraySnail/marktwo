  ...
  handleSearch(e) {
    ...
    if (!this.state.searchString) {
      ...
      searchResults = this.state.allLines
        ...
        .map(id => ({ id, html: marked(this.state.doc[id]).replace(searchRegex, m => `<mark>${m}</mark>`) }))
        .slice(0, 1000)
    } else {
      ...
      if (exactMatchRegex.test(this.state.searchString)) {
        searchRegex = new RegExp(
          this.state.searchString.match(exactMatchRegex)[1].replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&'),
          'ig'
        )
      } else {
        const keywords = this.state.searchString.split(' ').map(t => t.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&')) // comment out regex expressions
          ...
          <div className="m2-help modal is-active">
            ...
            <div className="modal-card">
              ...
              <section className="modal-card-body content">
                ...
                <ul>
                  <li>
                    When you select a block (paragraph), it automatically transforms that HTML into markdown, and when
                    you exit the block, it renders to HTML. Since Markdown can be multiline, you'll have to press enter
                    twice to exit a block.
                  </li>
                  <li>
                    MarkTwo continuously and efficiently syncs the document you're working via Google Drive. When the
                    edit indicator bar turns light blue (or dark pink in dark mode), it means changes are being made.
                    Once it turns dark blue (bright pink in dark mode), the changes are synced (a few seconds after
                    you're done editing).
                  </li>
                  <li>
                    We do not have access to your documents, they are as secure as your Google account (we recommend
                    enabling two factor authentication).
                  </li>
                  <li>
                    You can search for keywords, adding quotes matches "exact terms", and the special keyword #todo
                    searches for all undone checklist tasks.
                  </li>
                </ul>
...
1. The numbers themselves don't matter
    1. Again, four spaces starts a sub-list`}
                ...
                <p>
                  MarkTwo supports the standard syntax for todo lists. Additionally, it supports reminders&mdash;when a
                  not-done todo list item contains a reminder string, you'll get a banner reminder when you load MarkTwo
                  on or after that day. The format for a reminder string is :reminder-ribbon: [date][semi-colon].
                </p>
                <pre>
...
| entry 1  | entry2  |`}
                ...
                <h5>Block quotes</h5>
                <pre>{`> An angle bracket and a space will render a block quote.`}</pre>
...
\`\`\``}
                ...
                <h5>Bookmarks</h5>
                <p>
                  A line that starts with two slashes and a space gets rendered as a bookmark, and shows up by default
                  in the <code>Search</code> view.
                </p>
                <pre>{`// January notes`}</pre>

                <h5>Images</h5>
                <p>
                  Link to images across the web with standard markdown syntax, or upload your own with the{' '}
...
/image`}
                ...
                <h5>HTML</h5>
                <p>
                  The markdown spec is not intended to completely replace HTML. If you'd like a particular tag or style,
                  you can just include it as HTML and it will render. For example:
                </p>
                <pre>
...
<center>This will be centered</center>`}
                ...
                <h5>Text Tricks</h5>
                <p>
                  MarkTwo expands the strings <code>/today</code> and <code>/now</code> into the current date or date
                  and time for your locale. You can also use <code>/date</code> to bring up a date picker&mdash;this is
                  especially handy when you're setting reminders. Also, to make things easier to find later,{' '}
                  <code>#hashtags</code> and <code>@mentions</code> autocomplete. You can also search for and enter
                  emojis with colons like this: <code>:emojis:</code>
                </p>
                <pre>{`# Star date: /today
#hashtags, @mentions
:smiley_face:`}</pre>
