...
function initialize(gapi) {
  ...
  function getImagesFolder() {
    return gapi.client.drive.files.list({ q: `name='MarkTwo-Media'` }).then(response => {
  ...
  function find(name) {
    return gapi.client.drive.files.list({ q: `name='${name}'`, spaces: 'appDataFolder' }).then(response => {
  ...
  function getPagesForDoc(docId) {
    return new Promise((resolve, reject) => {
      gapi.client.drive.files
        .list({ q: `name contains '${docId}'`, spaces: 'appDataFolder', pageSize: 1000 })
        .then(response => {
          console.log(response)
  ...
  function deleteFile(name) {
    del(name)
    return new Promise(resolve => {
      gapi.client.drive.files.list({ q: `name='${name}'`, spaces: 'appDataFolder' }).then(response => {
        console.log(response)
        if (response.result.files.length) {
